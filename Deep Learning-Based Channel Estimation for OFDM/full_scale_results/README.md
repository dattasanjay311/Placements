...
- **`FULL_SCALE_RESULTS.md`** — the actual project findings, from training
  on the full-scale datasets (real 1K/10K/50K sizes, plus MNIST and
  CIFAR-10, full epoch budgets). Cite these numbers, not the demo run below.
- **`demo_reduced_scale_run/`** — output from a quick, reduced-scale
  smoke-test of this codebase (tiny datasets, 10 epochs, ~2 minutes on CPU).
  Useful for verifying the pipeline runs end-to-end, but too small/under-trained
  to draw research conclusions from — see the note at the top of
  `FULL_SCALE_RESULTS.md` for why the trends differ.
