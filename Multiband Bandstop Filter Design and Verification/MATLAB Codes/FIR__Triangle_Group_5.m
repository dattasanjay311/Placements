% Name : Datla Ravi Teja
% Roll No : 25M1064

clc;
clear;
close all;

f_samp = 650e3;    % 650 kHz


tolerance = 0.15;
f1_fp1 = 80e3;    % Lower passband edge
f1_fs1 = 85e3;    % Lower stopband edge
f1_fs2 = 115e3;   % Upper stopband edge
f1_fp2 = 120e3;   % Upper passband edge

% Normalized frequencies (digital domain)
f1_fp1_n = 2*f1_fp1/f_samp;
f1_fs1_n = 2*f1_fs1/f_samp;
f1_fs2_n = 2*f1_fs2/f_samp;
f1_fp2_n = 2*f1_fp2/f_samp;


f2_fp1 = 185e3;
f2_fs1 = 190e3;
f2_fs2 = 220e3;
f2_fp2 = 225e3;

% Normalized frequencies
f2_fp1_n = 2*f2_fp1/f_samp;
f2_fs1_n = 2*f2_fs1/f_samp;
f2_fs2_n = 2*f2_fs2/f_samp;
f2_fp2_n = 2*f2_fp2/f_samp;

function h = ideal_lp(cutoff, n)
    t = -floor(n/2):floor(n/2);
    h = (sin(cutoff * t)) ./ (pi * t);
    h(floor(n/2) + 1) = cutoff / pi; % handle divide-by-zero at center
end


n = 199;
if mod(n,2) == 0
    n = n + 1; % ensure odd length for symmetry
end

% We manually compute: w[k] = 1 - |k| / N, where k = -N:N, N = (n-1)/2
N = (n - 1)/2;
k = -N:N;
bartlett_window = 1 - abs(k) / N;

% Optional: visualize the window
figure;
stem(k, bartlett_window, 'filled');
title('Fejér–Cesàro / Bartlett / Triangular Window');
xlabel('Sample index k');
ylabel('Amplitude');
grid on;

bs_ideal1 =  ideal_lp(pi,n) -ideal_lp(((f1_fp2_n+f1_fs2_n)/2)*pi,n) + ideal_lp(((f1_fp1_n+f1_fs1_n)/2)*pi,n);
FIR_BandStop1 = bs_ideal1 .* bartlett_window;

fvtool(FIR_BandStop1);
title('Bandstop Filter 1 (Bartlett Window)');

[H,f] = freqz(FIR_BandStop1,1,1024,f_samp);
figure;
plot(f, abs(H));
title('Magnitude Response - Bandstop Filter 1');
xlabel('Frequency (Hz)');
ylabel('|H(f)|');
grid on;


bs_ideal2 =  ideal_lp(pi,n) -ideal_lp(((f2_fp2_n+f2_fs2_n)/2)*pi,n) + ideal_lp(((f2_fp1_n+f2_fs1_n)/2)*pi,n);
FIR_BandStop2 = bs_ideal2 .* bartlett_window;

fvtool(FIR_BandStop2);
title('Bandstop Filter 2 (Bartlett Window)');

[H,f] = freqz(FIR_BandStop2,1,1024,f_samp);
figure;
plot(f, abs(H));
title('Magnitude Response - Bandstop Filter 2');
xlabel('Frequency (Hz)');
ylabel('|H(f)|');
grid on;


FIR_BandStop = conv(FIR_BandStop1, FIR_BandStop2);

fvtool(FIR_BandStop);
title('Combined Bandstop Filter (Bartlett Window)');

[H,f] = freqz(FIR_BandStop,1,1024,f_samp);
figure;
plot(f, abs(H));
title('Magnitude Response - Combined Bandstop Filter');
xlabel('Frequency (Hz)');
ylabel('|H(f)|');
grid on;


bs_ideal =  ideal_lp(pi,n) -ideal_lp(((f2_fp2_n+f2_fs2_n)/2)*pi,n) + ideal_lp(((f2_fp1_n+f2_fs1_n)/2)*pi,n)-ideal_lp(((f1_fp2_n+f1_fs2_n)/2)*pi,n) + ideal_lp(((f1_fp1_n+f1_fs1_n)/2)*pi,n);;

FIR_BandStop3 = bs_ideal .* bartlett_window;

fvtool(FIR_BandStop3);
title('Direct Combined Bandstop Filter (Bartlett Window)');

[H,f] = freqz(FIR_BandStop3,1,1024,f_samp);
figure;
plot(f, abs(H));
title('Magnitude Response - Direct Combined Bandstop Filter');
xlabel('Frequency (Hz)');
ylabel('|H(f)|');
grid on;

figure;
plot(f, unwrap(angle(H)));
title('Magnitude Response - Direct Combined Bandstop Filter');
xlabel('Frequency (Hz)');
ylabel('|H(f)|');
grid on;

disp('h[n] array =')
% Print 5 numbers per line, with 3 decimal places
nCols = 5;
for i = 1:length(FIR_BandStop3)
    fprintf('%8.3f', FIR_BandStop3(i));  % Print each number with width=8, 3 decimals
    if mod(i, nCols) == 0 || i == length(FIR_BandStop3)
        fprintf('\n');  % Newline every 5 numbers or at the end
    end
end